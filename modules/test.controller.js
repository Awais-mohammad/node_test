...
const solution = () => {
  ...
  while (calculations.customersRemaining > 0) {
    if (calculations.prevDistance === 0) {
      ...
      routeObject.path = `${Math.round((calculations.dosfec[closest].distance.value) * 1609.344)} miles from Store to ${customers[closest]}.`
     ...
     */
      ...
      if (
        ...
        calculations.deliveryRoutes.push({ totalLength: `${Math.round((calculations.prevDistance) * 1609.344)} miles from Store.`, route: calculations.currentRoute });
